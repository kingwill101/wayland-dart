...
class Generator {
  ...
  Future<String> getInputFile(String file) async {
    ...
    if (file.startsWith('http')) {
      ...
      if (cacheFile.existsSync() && !force) {
        logLn('Using cached version of ${basename(file)}');
        return await cacheFile.readAsString();
      }

      logLn('Downloading ${basename(file)}...');
      ...
      if (event.args.isNotEmpty) {
        ...
        for (final arg in event.args) {
          ...
          switch (arg.type) {
            case 'int':
              output.writeln(
                  'final $argName = ByteData.view(data.buffer).getInt32(_offset, Endian.little);');
              output.writeln('_offset += 4;');
              break;
            case 'uint':
              output.writeln(
                  'final $argName = ByteData.view(data.buffer).getUint32(_offset, Endian.little);');
              output.writeln('_offset += 4;');
              break;
            case 'fixed':
              output.writeln(
                  'final $argName = fixedToDouble(ByteData.view(data.buffer).getInt32(_offset, Endian.little));');
              output.writeln('_offset += 4;');
              break;
            case 'string':
              output.writeln(
                  'final ${argName}Length = ByteData.view(data.buffer).getUint32(_offset, Endian.little);');
              output.writeln('_offset += 4;');
              output.writeln(
                  'final $argName = utf8.decode(data.sublist(_offset, _offset + ${argName}Length - 1));');
              output.writeln(
                  '_offset += ${argName}Length; // Skip the string bytes and null terminator');
              output
                  .writeln('while (_offset % 4 != 0) { _offset++; } // Padding');
              break;
            case 'object':
            case 'new_id':
              output.writeln(
                  'final $argName = innerContext.getProxy(ByteData.view(data.buffer).getUint32(_offset, Endian.little)).objectId;');
              output.writeln('_offset += 4;');
              break;
            case 'array':
              output.writeln('final $argName = getArray(data, _offset);');
              output.writeln(
                  'var arrayLength = ByteData.view(data.buffer).getUint32(_offset, Endian.little);');
              output.writeln('_offset += 4 + arrayLength;');
              output
                  .writeln('while (_offset % 4 != 0) { _offset++; } // Padding');
              break;
            case 'fd':
