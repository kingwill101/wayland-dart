...
class ScanCommand extends Command {
  ...
  ScanCommand() {
    argParser
      ...
      ..addFlag('force',
          help: 'Force re-download of all protocol XMLs (bypass cache)',
          defaultsTo: false)
      ..addFlag('list-protocols',
          help: 'List all protocols defined in the YAML file',
          defaultsTo: false);
  }
  ...
  void run() async {
    ...
    bool listProtocols = argResults?['list-protocols'] ?? false;
    ...
    if (argResults?['protocols'] != null) {
      ...
      if (listProtocols) {
        print('Protocols defined in ${argResults?['protocols']}:');
        print('=' * 60);
        for (var i = 0; i < protocolsList.length; i++) {
          final protocolMap = protocolsList[i] as YamlMap;
          final name = protocolMap['name'] ?? 'unknown';
          final input = protocolMap['input'] ?? 'unknown';
          final output = protocolMap['output'] ?? 'unknown';
          print('${i + 1}. $name');
          print('   Input:  $input');
          print('   Output: $output');
          if (protocolMap['dependencies'] != null) {
            print('   Deps:   ${protocolMap['dependencies']}');
          }
          print('');
        }
        print('Total: ${protocolsList.length} protocols');
        return;
      }
      ...
      var generators = protocolsList.map((protocol) {
        ...
        final protocolPrefix =
            (protocolMap['prefix'] as String?) ?? prefix;
        final protocolSuffix =
            (protocolMap['suffix'] as String?) ?? suffix;

        return Generator(
            packageName: packageName,
            prefix: protocolPrefix,
            suffix: protocolSuffix,
